...
export function getNoteListNearMonth(context,{Year,Month}){
    return new Promise((resolve,reject)=>{
        context.state.database.transaction(tx => {
            ...
            tx.executeSql(`select Id,Title,Category,Content,CreateTime,State,Year,Month,Day,Sort from note where (Year = ${preData.Year} and Month=${preData.Month} or Year = ${Year} and Month=${Month} or Year = ${nextData.Year} and Month=${nextData.Month}) order by Year desc,Month desc`, [], (tx, res) => {
                var rowData = [];
                for (var x = 0; x < res.rows.length; x++) {
                    rowData.push(res.rows.item(x));
                }
